                                       ...
                                       struct mailbox *box)
{
    ...
    if (box != NULL)
    {
        ...
        if (fts_mailbox_get_guid(box, &box_guid) < 0)
        {
            vi("fts_elastic: update_set_mailbox: fts_mailbox_get_guid failed");
            _ctx->failed = TRUE;
        }
        struct elastic_fts_field *field;
...
static int fts_backend_elastic_rescan(struct fts_backend *_backend)
{
    ...
    const enum mailbox_list_iter_flags iter_flags =
        (enum mailbox_list_iter_flags)(MAILBOX_LIST_ITER_NO_AUTO_BOXES |
